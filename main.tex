...
Пусть $v_1, \ldots, v_n$ -- начальные элементы, $u_1, \ldots, u_m$ -- узлы унара $X$, которых по лемме~\ref{lemma:6} конечное число, $t_i = \indeg u_i$ $(i = 1,2,\ldots, m)$.
...
\begin{proof}
	...
	\textbf{Это не так - вот контрпример ниже.}
	\begin{figure}[ht!]
		\centering
		\includegraphics[scale=0.07]{contrexample.jpg}
		\caption{Контрпример. Компонента связности $Y$.}
		\label{fig:contrexample}
	\end{figure}
